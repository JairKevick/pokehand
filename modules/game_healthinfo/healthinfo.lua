...
function onSoulChange(localPlayer, soul)
  soulLabel:setText(tr('Catch') .. ': ' .. soul)
end

function onFreeCapacityChange(player, freeCapacity)
  capLabel:setText(tr('Pokes') .. ': ' .. freeCapacity)
end
...
function onPokeHealth(text)
local h = text:explode(",") 
local pokeHealthBar = healthInfoWindow:recursiveGetChildById("pokeHealthBar")
local hp, maxHp = tonumber(h[2]), tonumber(h[3])
...
if pokeHealthBar:getText() == "0 / 0" then
pokeHealthBar:setText("No Pokemon")
end
end
